...
После создания файлов форм **обязательно** обнови файл объекта (`object_path/object_file`). Для всех объектов, кроме `Отчет`, добавь секцию `Интерфейс`; для `Отчет` добавь свойства на верхнем уровне.
...
Правила:
- Для объектов, кроме `Отчет`, если уже есть секция `Интерфейс` — обнови только нужные поля, не перезаписывай остальные
