...
Секция добавляется **сразу после строки `ОбластьВидимости`** (или после `Ид`/`Имя`, если `ОбластьВидимости` отсутствует), до `Реквизиты`.
...
- `ИспользоватьСозданиеПриВводе: Истина` — только для `Справочник`; для `Документ` и других типов не добавляй
