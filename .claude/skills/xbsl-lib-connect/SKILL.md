...
- Массив **пустой** → проверить `Проект.yaml` в клоне на наличие `ВидПроекта: Библиотека`
  - Есть → перейти к **Шагу 1.5** (проверка в облаке)
...
```
Если найдены → выбрать (как в п. A). Иначе — проверить `Проект.yaml` и перейти к **Шагу 1.5**.

---

## Шаг 1.5 — Проверить, не загружена ли библиотека в облако

Этот шаг выполняется только для вариантов A и C (GitHub/папка с исходниками), когда
готовый `.xlib` не найден и нужна сборка.

Прочитать `Проект.yaml` из источника и извлечь:
```bash
grep -E "^Имя:|^Поставщик:" <путь_к_папке_с_Проект.yaml>/Проект.yaml
```
Сохранить как `LIB_NAME` и `LIB_VENDOR`.

Запросить список проектов в облаке и найти совпадение:
```bash
python3 .claude/skills/xbsl-deploy/scripts/api.py --action list-projects
```

Отфильтровать результат: `project-kind == "Library"` и `name == LIB_NAME`.

**Если совпадение найдено:**

Сообщить пользователю:
> Библиотека **{LIB_NAME}** (поставщик: {LIB_VENDOR}) уже загружена в облако (id: {id}).
> Что делать?
> 1. **Использовать существующий проект** — пропустить сборку и загрузку, перейти к выпуску релиза
> 2. **Загрузить новую версию** — собрать и загрузить новую сборку поверх существующего проекта

- Если **вариант 1**: перейти к Шагу 6, пропустив Шаги 2А–5.
  `LIB_VERSION` = `assembly-version` последней сборки из `list-builds --project-id {id}`.
- Если **вариант 2**: продолжить Шаг 2А (собрать), затем в Шаге 5 передать `--project-id {id}`.

**Если совпадение не найдено:**

Продолжить Шаг 2А (сборка из исходников).
