...
метод {handler}(Запрос: HttpСервисЗапрос)
    ...
    поймать ОшибкаОбработкиЗапроса: Исключение
        ОбработатьОшибку(Запрос.Ответ, ОшибкаОбработкиЗапроса)
    ;
;"""
...
def _xbsl_error_helper() -> str:
    return """\
метод ОбработатьОшибку(Ответ: HttpСервисОтвет, ОшибкаОбработкиЗапроса: Исключение)
    Ответ.УстановитьКодСтатуса(500)
    Ответ.Заголовки.Установить("Content-Type", "text/plain; charset=utf-8")
    Ответ.УстановитьТело(ОшибкаОбработкиЗапроса.Описание)
;"""
