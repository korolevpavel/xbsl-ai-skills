...
⚠️ Обработчики записываются в `<Имя>.Объект.xbsl` — с суффиксом `.Объект`, иначе платформа не привяжет код к объекту. Если `rls_state` вернул `xbsl_file: null` — создай файл с суффиксом `.Объект`.
...
- Если `xbsl_exists = false` — создай `<ИмяОбъекта>.Объект.xbsl` (новый файл).
