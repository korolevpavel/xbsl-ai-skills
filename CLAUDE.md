...
.claude/skills/          # скиллы Claude Code
    ...
    xbsl-collab-add/         # интеграция Системы взаимодействия (P1a/P1b/P2/P3)
        scripts/find_app.py  # находит компонент приложения и состояние СВ
        references/patterns.md  # шаблоны YAML/XBSL для P1a, P1b, P2, P3
...
### xbsl-collab-add
Интегрирует Систему взаимодействия (1С-диалог) в приложение. Паттерны: P1a — вкладка «Обсуждение» с `КомпонентОбсуждения` (автоматический показ); P1b — вкладка «Чат» с кнопкой «Открыть чат» и реквизитом `ПоказатьЧат: Булево`; P2 — группа «Взаимодействие» в панели навигации (навигационные команды управления СВ); P3 — веб-чат виджет (`ВебЧат: {Использовать: Истина}` + обработчик). Оркестрирует xbsl-form-info (P1). `scripts/find_app.py` — находит компонент приложения и проверяет текущее состояние СВ (P2/P3).
