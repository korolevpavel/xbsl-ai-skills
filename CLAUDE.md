...
.claude/skills/          # скиллы Claude Code
    ...
    xbsl-yandex-maps/       # добавление Яндекс.Карты в форму через внешнюю библиотеку
        references/карта-форма-объекта.md  # YAML + XBSL шаблоны
...
### xbsl-yandex-maps
Добавляет интерактивную Яндекс.Карту в форму объекта через внешнюю библиотеку. Нативная поддержка карт в платформе не реализована. Алгоритм: предупредить пользователя → предложить два варианта (подключить библиотеку / ничего не делать) → если выбрана библиотека: проверить наличие в `Проект.yaml`, при необходимости вызвать `xbsl-lib-connect` (дефолтный источник: https://github.com/korolevpavel/xbsl-yandex-maps) → запустить xbsl-form-info для анализа объекта → добавить `КонтейнерHtml` в `ФормаОбъекта.yaml` и обработчик `ПослеСоздания` в `.xbsl`. Технически использует `Компоненты.КартаЯндекс.Содержимое = ЯндексКарты.СформироватьHtmlКарты(Объект.Адрес)`.
