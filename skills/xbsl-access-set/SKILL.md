...
description: >
  ...
  РегистрСведений, РегистрНакопления, HttpСервис, Обработка (10.0+). Вызывай при запросах «настрой права
...
```bash
{python} skills/xbsl-access-set/scripts/access_state.py \
...
Для `Обработка` сводка дополнительно содержит `processing_operations`:
`effective_call`, `source` (`operation`, `processing`, `platform`) и
`rename_requires_recalculation`. Собственный `КонтрольДоступа` операции
заменяет настройку обработки для этой операции.

Для изменения права одной операции добавь к dry-run `--object <ИмяОбработки>
--operation <ИмяОперации> --set РазрешеноАдминистраторам`. После проверки
плана тот же вызов с `--apply` меняет только право `Вызов` этой операции.
...
Запросы на ручную выдачу или отзыв экземпляров ключа доступа также направляй в
`xbsl-pattern-rls`; `access_state.py` и список поддерживаемых типов не расширяй.
...
| `--operation <Имя>` | Выбрать операцию одной Обработки для установки `Вызов`; требует `--object` и `--set` |
...
- Обработка (10.0+): обычный `--set` меняет только общее `ПоУмолчанию`;
  `--operation` меняет только `Операции[].КонтрольДоступа.Разрешения.Вызов`.
  CLI устанавливает для обработки только статические права. Вычисляемые
  права с обработчиком настраивай по `xbsl-meta-add/references/Обработка.md`.
- При переименовании операции с собственным `КонтрольДоступа` вызови
  `ИмяОбработки.ПересчитатьРазрешенияДоступа()` в обработчике обновления проекта.
  Сводка помечает такие операции `rename_requires_recalculation: true`.
