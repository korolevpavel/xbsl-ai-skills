...
| подключить / обновить / включить / выключить расширение | **L: Управление расширением** |

## Шаг 2: Получи токен

```bash
{python} skills/xbsl-deploy/scripts/api.py --action get-token
```
...
Если пользователь указал локальную папку проекта — прочитай `Проект.yaml` и проверь `ВидПроекта`:
```bash
...
Если найдено `ВидПроекта: Расширение` — перейди к сценарию L. Расширение
подключается к существующему приложению и не создаёт отдельное приложение.

### A1. Определи проект
Если `ELEMENT_PROJECT_ID` не задан — выведи список проектов и попроси пользователя выбрать:
```bash
{python} skills/xbsl-deploy/scripts/api.py --action list-projects
```
...
```bash
{python} skills/xbsl-deploy/scripts/api.py --action list-spaces
```
...
Если проект создан загрузкой сборки в сценарии H, используй `id` сборки из
ответа `upload-build` как `assembly-id`. У нового проекта может ещё не быть
сборки по умолчанию, поэтому передавай точную версию:

```bash
{python} skills/xbsl-deploy/scripts/api.py --action create-app --name <name> --space-id <space-id> --version-id <assembly-id>
```

Для уже существующего проекта, когда конкретная сборка не выбрана, сохрани
прежний вариант:

```bash
{python} skills/xbsl-deploy/scripts/api.py --action create-app --name <name> --space-id <space-id> --project-id <project-id>
```
...
```bash
{python} skills/xbsl-deploy/scripts/api.py --action start-app --app-id <app-id>
```

### A5. Жди Running
Опрашивай каждые 10 сек, до 5 мин:
```bash
{python} skills/xbsl-deploy/scripts/api.py --action get-app --app-id <app-id>
```
...
После создания из сборки дополнительно проверь, что `source.project-version-id`
совпадает с `assembly-id` из H4. Старый `Running` для другой сборки не доказывает
успех новой публикации.
...
```bash
{python} skills/xbsl-deploy/scripts/api.py --action list-apps --name <name>
```
...
```bash
{python} skills/xbsl-deploy/scripts/api.py --action create-dump --app-id <app-id>
```
Сохрани `id` из ответа как `dump-id`. Проверь статус **сразу** (без паузы), затем опрашивай каждые 3 сек (до 60 сек):
```bash
{python} skills/xbsl-deploy/scripts/api.py --action get-dump --app-id <app-id> --dump-id <dump-id>
```
...
### B4. Найди ветку
```bash
{python} skills/xbsl-deploy/scripts/api.py --action list-branches --project-id <project-id> --branch-name <branch>
```

### B5. Привяжи ветку к приложению (если нужно)
Если `branch.application.id != app-id`:
```bash
{python} skills/xbsl-deploy/scripts/api.py --action update-branch --branch-id <branch-id> --app-id <app-id>
```

### B6. Перезапусти (подхвати изменения из репозитория)
Сначала останови (если приложение не `Stopped`):
```bash
{python} skills/xbsl-deploy/scripts/api.py --action stop-app --app-id <app-id>
```
...
```bash
{python} skills/xbsl-deploy/scripts/api.py --action delete-app --app-id <app-id>
```

Если ветка создавалась скиллом — предложи удалить и её. Удаляй ветку только после отдельного подтверждения:
```bash
{python} skills/xbsl-deploy/scripts/api.py --action delete-branch --branch-id <branch-id>
```
...
Затем выполни merge:
```bash
{python} skills/xbsl-deploy/scripts/api.py --action merge-branch --branch-id <branch-id>
```
Сообщи результат.
...
```bash
{python} skills/xbsl-deploy/scripts/api.py --action upload-build \
  --file <path> \
  --space-id <space-id> \
  [--branch-name <branch>] \
  [--commit-id <hash>] \
...
Из ответа возьми `project-id` и `id` загруженной сборки. Если следующим шагом
создаёшь приложение, перейди к A3 и передай именно `id` сборки через
`--version-id`; одного `--project-id` для нового проекта недостаточно.
...
Если найдено `ВидПроекта: Расширение` — перейди к сценарию L. Не запускай
`deploy.py`, `build.py --kind application`, `project-update` основного проекта.

**Путь 1** (из исходников): нужны `ELEMENT_APP_ID`, `ELEMENT_PROJECT_ID`.
```bash
{python} skills/xbsl-deploy/scripts/deploy.py [--project-dir PATH] [--branch B] [--commit C]
```
...
```bash
{python} skills/xbsl-deploy/scripts/deploy.py --from-branch [--branch-id ID]
```
...
```bash
{python} skills/xbsl-deploy/scripts/api.py --action get-technology-version --app-id <app-id>
```
...
```bash
{python} skills/xbsl-deploy/scripts/api.py --action update-technology-version \
...
```bash
{python} skills/xbsl-deploy/scripts/api.py --action get-group-task --task-id <task-id>
```
...
```bash
{python} skills/xbsl-deploy/scripts/api.py --action create-app \
...
## Сценарий L: Расширение Element 10.0

`ВидПроекта: Расширение` имеет отдельный жизненный цикл. До действий проверь
`РасширяемыеПроекты`: все перечисленные проекты должны быть установлены в
целевом приложении, а их версии — входить в заданные диапазоны. Отсутствие
списка означает универсальное расширение.

1. Собери проект расширения в среде разработки Element 10.0 и выбери сборку
   расширения. `build.py` и `deploy.py` не формируют и не загружают такую
   сборку: формат архива расширения не подтверждён в опубликованной
   документации. При попытке запуска сборщика покажи диагностику
   `xbsl-deploy.extension_archive_unsupported`.
2. В панели управления открой **Приложения** → нужное приложение → вкладка
   **Расширение** → **Обновить**. Подключи нужную сборку расширения, укажи
   порядок применения и нажми **Далее**.
3. Включи **Проверить ошибки применения проекта**, затем **Начать обновление**.
   Для управления несколькими приложениями используй карточку проекта
   расширения → меню сборки → **Установить в приложения**.
4. Для дальнейших изменений в форме обновления можно поменять сборку,
   порядок расширений, включить, выключить или отключить расширение.
   Каждая из этих операций запускает процедуру обновления приложения.

[Структура расширения](https://1cmycloud.com/console/help/element/10.0/docs/topics/extension-project-structure/),
[управление в приложении](https://1cmycloud.com/console/help/element/10.0/docs/topics/manage-application-extensions/),
[установка в несколько приложений](https://1cmycloud.com/console/help/element/10.0/docs/topics/manage-extensions-in-applications/).
...
- При HTTP- или транспортной ошибке `api.py` печатает JSON-диагностику и
  завершает команду с ненулевым кодом; не переходи к следующему шагу деплоя
  после такого ответа.
