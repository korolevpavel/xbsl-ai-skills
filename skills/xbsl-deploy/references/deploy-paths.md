...
```bash
{python} skills/xbsl-deploy/scripts/deploy.py \
  [--project-dir PATH] \
  [--app-id APP_ID] \
...
- Включаются только: `.yaml`, `.xbsl`, `.xbql`, `.md`, `.txt`, а также
  файлы SOAP-клиента `*.Wsdl.N` и `*.Xsd.N` (положительный номер `N`).
  При упаковке в `.xasm` к WSDL добавляется суффикс `.wsdl`, который ожидает
  компилятор. XSD сохраняет исходное имя. Имена файлов в исходном проекте не
  меняются. Разрешение внешнего импорта XSD в облачной сборке проверяется в
  [#194](https://github.com/korolevpavel/xbsl-ai-skills/issues/194).

**Dry-run** (только сборка, без деплоя):
```bash
{python} skills/xbsl-deploy/scripts/deploy.py --dry-run
```
...
```bash
{python} skills/xbsl-deploy/scripts/api.py \
  --action sync-branch \
  --app-id APP_ID \
  --branch-id BRANCH_ID
```
