...
На target 10.0+ перед сценарием загрузки/выгрузки файла прочитай `references/file-names-10.md`: недопустимые символы в имени обрабатываются режимом всего приложения. Глобальный режим меняй только по явному запросу; при `НеИсправлять` предусмотрена ошибка имени.
