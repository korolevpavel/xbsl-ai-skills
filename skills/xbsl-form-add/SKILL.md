...
- При запросе настроек «Избранного» на target 10.0+ → прочитай `references/favorites-10.md`; в обычные формы свойство не добавляй
...
- Для `КомпонентОбсуждения.ОтображатьЗаголовок`, программного выбора раздела
  `ШаблонФормыСРазделами` или вложенных узлов встроенных веб-страниц на target
  10.0+ читай `references/ui-deltas-10.md`. Встроенные веб-страницы размещаются
  в компоненте приложения (`Приложение.yaml`), а не в форме объекта; если
  пользователь просит кастомное меню без существующей команды или компонента,
  оставь стандартное меню и сообщи, какие элементы нужно определить.
