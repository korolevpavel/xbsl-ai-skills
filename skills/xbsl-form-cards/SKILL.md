...
```bash
{python} skills/xbsl-form-cards/scripts/generate.py --object <ИмяОбъекта> --root <корень>
```
...
```bash
{python} skills/xbsl-form-cards/scripts/generate.py --object <ИмяОбъекта> --root <корень> --apply
```
...
Для целевой версии 10.0 при необходимости добавь `--settings-mode Расширенный`, `--auto-filter <Поле>[:Всегда|Опционально|НеОтображать]`, `--auto-sort <Поле>`, `--deletion-filter`, `--multi-sort`. Флаги полей можно повторять. Их размещение и YAML-паттерн описаны в `references/patterns.md`.
