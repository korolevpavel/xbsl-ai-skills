...
Перед созданием `ГлавнаяСтраница.xbsl` прочитай `docs/xbsl-spec.md`.
...
```bash
{python} skills/xbsl-explore/scripts/explore.py \
...
- Если пользователь просит встроить веб-страницы в компонент приложения на
  target 10.0+ → `../xbsl-form-add/references/ui-deltas-10.md` и
  `references/регистрация.md`. Записи `ВстроенныеВебСтраницы` и
  `ИнтерфейсВстроенныхВебСтраниц` относятся к `Приложение.yaml`, не к
  `ГлавнаяСтраница.yaml`.
