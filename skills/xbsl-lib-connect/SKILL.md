...
**Вспомогательный скрипт:** `skills/xbsl-lib-connect/scripts/lib_connect.py`
...
Найти `.xlib` в клоне:
```bash
{python} skills/xbsl-lib-connect/scripts/lib_connect.py \
...
```bash
{python} skills/xbsl-deploy/scripts/build.py \
...
```bash
[ -n "$TMP_DIR" ] && {python} skills/xbsl-lib-connect/scripts/lib_connect.py \
