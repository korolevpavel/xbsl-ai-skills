...
веб-панель управления. Скилл выполняет всё вокруг этого шага и сверяет фактическую
версию релиза с версией сборки. В 10.0 номер релиза нельзя назначить вручную.
...
- `expected_release_version` → ожидаемая версия релиза из `Version` сборки без
  числового суффикса (`1.0-5` → `1.0`). Для готовой релизной `.xlib` фактическое
  значение берётся из `release_version`, а не пересчитывается.
...
Если `HAS_RELEASE` из Шага 2 равно `true` — **пропустить выпуск релиза**.
`RELEASE_VERSION` уже установлен из метаданных готовой `.xlib` (`release_version`).
Проверь его формат и перейди к Шагу 7.
...
> 5. Проверьте показанную версию: панель определяет её из сборки без суффикса; изменить её вручную нельзя. Ожидается `{expected_release_version}`.
> 6. Нажмите **Выпустить релиз** и проверьте фактическую версию в списке релизов.
>
> Сообщите, что релиз выпущен, и его фактическую отображаемую версию, если у меня нет доступа к панели.

Прочитай фактическую версию в панели самостоятельно, если доступ есть. Сверь её с
`expected_release_version`. Если они различаются, выясни причину до изменения
`Проект.yaml`. Только подтверждённую фактическую версию сохрани в `RELEASE_VERSION`.
...
```
При `valid: false` — не записывать зависимость, проверить выбранную сборку и
фактический релиз в панели.
...
Проверь, что `Библиотеки[].Версия` для `{LIB_VENDOR}::{LIB_NAME}` в `after`
равна именно `RELEASE_VERSION` готовой `.xlib` или выпущенного релиза.
