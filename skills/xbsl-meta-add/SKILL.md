...
reference-paths и статусов для 1С:Предприятие.Элемент 10.0.
...
- `partial` — не генерируй объект по догадке; сообщи, что полная поддержка
  пока не описана в registry, и используй reference contract только для
...
Если пользователь просит настроить «Избранное» у `Справочник`, `Документ`, `ХранилищеНастроек` или `ПланОбмена` на target 10.0+, дополнительно прочитай `references/Избранное.md`. В базовый шаблон свойство не добавляй.
