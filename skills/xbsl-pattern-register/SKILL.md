...
description: >
  ...
  Для режима 10.0 `ПодчинениеРегистратору` применяй отдельный паттерн D1.
...
Перед написанием обработчика прочитай `docs/xbsl-spec.md`, затем профильный
референс с паттернами регистра.
...
```bash
{python} skills/xbsl-pattern-register/scripts/extract_meta.py <путь-к-регистру.yaml>
{python} skills/xbsl-pattern-register/scripts/extract_meta.py <путь-к-документу.yaml>
```
...
- для РС `writing_mode`, `registrar_type`, `filter_fields` определяют независимый или подчинённый паттерн
...
| Записать движения документа в РС с `РежимЗаписи: ПодчинениеРегистратору` (10.0+) | D1 |
...
- **Независимый РС:** фильтр по измерениям, стандартного регистратора нет
- **Подчинённый РС (10.0+):** фильтр содержит только `Регистратор` и устанавливается до записи; `Ведущее` у измерений не поддерживается
