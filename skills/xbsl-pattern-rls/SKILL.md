...
description: >
  ...
  Создаёт необходимые артефакты: КлючДоступа (.yaml и, для автоматического режима, .xbsl),
...
Перед созданием или дополнением `.xbsl` прочитай `docs/xbsl-spec.md`.
...
⚠️ RLS-обработчики записываются в `<Имя>.xbsl` (без суффикса `.Объект`). Файл `<Имя>.Объект.xbsl` предназначен только для lifecycle-событий (`ПередЗаписью`, `ПослеЗаписи`) — туда RLS не кладётся. Если `rls_state` вернул `xbsl_file: null` — создай файл `<ИмяОбъекта>.xbsl`.

Для разграничения модулей: у документа обработчик перед записью имеет документированную
сигнатуру (точное имя типа замени на имя объекта):

```xbsl
@Обработчик
метод ПередЗаписью(
    До: <ИмяОбъекта>.Данные,
    ПараметрыЗаписи: <ИмяОбъекта>.ПараметрыЗаписи)
;
```

Сам RLS-workflow не создаёт lifecycle-модуль и этот обработчик. Если отдельный запрос
требует lifecycle-обработчик, сначала сверь сигнатуру для конкретного вида объекта
через `xbsl-docs` и используй профильный skill.
...
4. **Режим выдачи**:
   - `automatic` — default: `РучнаяВыдача: Ложь` задаётся явно в YAML target 10.0, handler и системные/явные пересчёты;
   - `automatic-disabled` — handler сохраняется, YAML содержит
     `ОтключитьСистемныеПересчеты: Истина`, прикладные события явно вызывают пересчёт;
   - `manual` — YAML содержит `РучнаяВыдача: Истина`, key-handler и вызовы
     `ПересчитатьКлючи()`/`Пересчитать()` не создаются; выдача и отзыв выполняются
     прикладным кодом.
...
Для собственного ключа нужен **1 UUID на объект** и по одному UUID на каждый
пользовательский параметр. Системный параметр `Владелец` содержит только `Имя`
и `Тип`, без `Ид`. Вызови `xbsl-uuid` с соответствующим числом.
...
В режимах `automatic` и `automatic-disabled` создай `<ИмяКлюча>.xbsl` в той же
папке — шаблон `ПроверитьНаличиеКлючейДоступа`. В режиме `manual` этот файл с
handler не создавай; companion с другими бизнес-методами допустим.

Для `automatic-disabled` зафиксируй документированные прикладные события,
которые вызывают пересчёт: ответственность за актуальность ключей переходит к
приложению. Для `manual` добавь в подходящие бизнес-события паттерны `Выдать`,
`Отозвать`, `ОтозватьКлючи` или bulk-выдачу из reference.
...
Создано:
  ...
  <object_path>/<КлючИмя>.xbsl   — обработчик ПроверитьНаличиеКлючейДоступа  [P2/P2-б automatic*]
...
**Своевременный пересчёт при изменении данных** (только P2 automatic*) — в `ПослеЗаписи` объекта-источника
...
- [ ] Для P2, P2-б automatic*: написан `ПроверитьНаличиеКлючейДоступа` с TODO-комментариями
- [ ] Для P2 manual: handler и пересчёты ключа отсутствуют, добавлены issue/revoke-события
- [ ] Пользовательские параметры имеют UUID; системный `Владелец` — без `Ид`
